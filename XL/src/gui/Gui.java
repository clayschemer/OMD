...
public class Gui extends JFrame implements Printable {
    ...
    private Storage storage = new Storage(ROWS, COLUMNS);
    ...
    private Gui(GuiList guiList, int count) {
        ...
        CurrentView currentView = new CurrentView(storage);
        statusPanel.add(WEST, currentView);
        statusPanel.add(CENTER, statusArea);
        // sheetPanel
        BorderPanel sheetPanel = new BorderPanel();
        SheetView sheetView = new SheetView(ROWS, COLUMNS, storage);
        sheetPanel.add(WEST, new RowNumberPanel(ROWS));
        sheetPanel.add(CENTER, sheetView);
        // gui
        Editor editor = new Editor(storage, statusArea);
        add(NORTH, statusPanel);
        add(CENTER, editor);
        add(SOUTH, sheetPanel);
        setJMenuBar(new XLMenuBar(this, guiList, statusArea, storage));
        addWindowListener(new ExitListener());
        pack();
        setResizable(false);
        setVisible(true);
        storage.setCurrent("A1");
    }
